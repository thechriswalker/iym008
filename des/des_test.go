...
var encryptDESTests = []CryptTest{
	{
		//133457799BBCDFF1
		[]byte{0x13, 0x34, 0x57, 0x79, 0x9b, 0xbc, 0xdf, 0xf1},
		// 0123456789abcdef
		[]byte{0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef},
		//85E813540F0AB405
		[]byte{0x85, 0xe8, 0x13, 0x54, 0x0f, 0x0a, 0xb4, 0x05},
	},
	{
...
func TestDESEncryptBlock(t *testing.T) {
	for i, tt := range encryptDESTests[:1] {
		c := NewDES(tt.key)
		cipher := c.EncryptBlock(tt.in)
		if !bytes.Equal(cipher, tt.out) {
			t.Errorf("#%d: result: %x want: %x", i, cipher, tt.out)
		}
	}
}
